...
handler.setFormatter(MoscowTimeFormatter(LOG_FORMAT))  # ← Теперь с уровнем лога
...
def get_local_time_str():
    """Получить текущее время в строке по МСК"""
    ...
    try:
        ...
        async with session.get(url, timeout=15) as response:
            ...
            if response.status == 200:
                ...
                if isinstance(data, dict) and "symbols" in data:  # ← Исправлено: добавлено 'data'
